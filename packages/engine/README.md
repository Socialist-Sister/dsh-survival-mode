...
**独立存档 + 文件重生点**：世界状态（天数/经验/成就/背包/生命/饥饿）随会话落盘（`world.json`）——同一会话重启后完整恢复进度，但**不跨会话**：每个新会话从第 1 天、0 经验、空背包开始。死亡分档：**普通难度软回退**（文件回退到最近备份 + 状态从 `respawn.json` 恢复到重生点时刻，成就保留，会话继续）；**hardcore 删档**（die → 遗言 → 会话终结）。会话开始时引擎自动把工作区 + 对话摘要（`conversation.md`）快照到 `${DSH_HOME}/survival-respawns/<会话id>/`（出生点）；夜晚睡觉或白天休息（`survival_sleep`，白天不跳夜）都覆盖快照（新重生点）；死亡时工作区回退到最近一次快照（重生点之后的文件改动丢失）。只对顶层会话生效，子代理的死亡不碰文件。快照排除可再生生成物（`node_modules` / `.git` / `dist` 等，可用 settings `respawnExcludes` 覆盖）。跃迁播报双通道：重要状态（入夜/低饥饿/低血量/死亡/备份完成）用 `agent.steer` 插队立即送达，普通播报（黎明战报）用 `agent.followup` 排队。
